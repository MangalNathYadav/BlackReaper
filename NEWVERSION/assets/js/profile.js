...
async function loadUserProfile(userId) {
  try {
    ...
    const userData = await database.getUserProfile(userId);
    ...
    if (!userData) {
      ...
      const newUserData = await database.getUserProfile(userId);
      if (!newUserData) {
...
async function saveProfileChanges(userId) {
  try {
    ...
    await database.updateProfileDetails(userId, { 
      bio, 
      status,
      avatar: avatarUrl 
    });
    ...
    if (editName) {
      ...
      if (displayName) {
        ...
        await database.updateUserProfile(userId, { displayName });
      }
    }
...
function changeProfileAvatar(userId) {
  ...
  database.updateProfileDetails(userId, { avatar: avatarUrl })
    .then(() => {
