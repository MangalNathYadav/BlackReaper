...
function addTaskToUI(taskId, task) {
  ...
  checkbox.addEventListener('change', async () => {
    ...
    if (completed) {
      ...
      // Animate and move completed task to bottom
      taskElement.classList.add('animating');
      setTimeout(() => {
        taskElement.classList.remove('animating');
        // Move to bottom
  ...
  // Insert completed tasks at the bottom, others at the top
  if (task.completed) {
    taskList.appendChild(taskElement);
  } else {
...
function addTaskToUI(taskId, task) {
  const taskList = document.getElementById('task-list');
  if (!taskList) {
    console.error('Task list container not found. Cannot add task to UI.');
    return;
  }
